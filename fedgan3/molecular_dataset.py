...
def data_iid(dataset, num_users):
    ...
    user_samples.extend([num_items] * num_users)
...
def data_noniid(dataset, num_users):
    ...
    for j in range(num_users):
        ...
        for rand in rand_set:
            ...
            num_samples = abs(idxs[int(prev*num_mols)] - idxs[int(rand*num_mols)])
    ...
    #     for rand in rand_set:
    #         dict_users[j] = np.concatenate((dict_users[j], idxs[rand*num_mols:(rand+1)*num_mols]), axis=0)
